	...
	|	                                                      		|
	|	MINEDU - Base de Proy Potenciales Brecha Cerrada - PRONIED	|
	...
	* ------------------------
	use 	"$Input\CUIMod_Inv_251110.dta", clear
	...
	* 2.1. Solo considerar inversiones de de GN - PRONIED
	* ----------------------------------------------------
 	gen 	crite_2 = 1 if uei == "PROGRAMA NACIONAL DE INFRAESTRUCTURA EDUCATIVA"
 	keep 	if crite_2 == 1
	...
	* ---------------------------------
	...
	merge 	m:1 cod_local using "$Input\LE_Activos_251106.dta", keep(1 3) keepusing(region prov dist) gen(_aux1)
	...
	* Verificar si ya fueron validados por envío de información PRONIED de Oct-2025 y descartar para lista actual.
	merge 	m:1 cod_mod anexo cui using "$Input\Pronied_2510_1.dta", keepusing(cod_local cod_mod cui)
	keep	if _merge == 1
	drop 	_merge
	
	merge 	m:1 cod_mod anexo cui using "$Input\Pronied_2510_2.dta", keepusing(cod_local cod_mod cui)
	keep	if _merge == 1
	drop 	_merge
	...
	compress
	save 	"LEModCUI_CrBr_PRONIED.dta", replace
	
	* Exportar a excel
	use 	"LEModCUI_CrBr_PRONIED.dta", clear
	
	order 	cod_local cod_mod region prov dist cui nombre_inversion des_tipo_formato nivel estado situacion uei 
	keep 	cod_local cod_mod region prov dist cui nombre_inversion des_tipo_formato nivel estado situacion uei		
	export 	excel using "Anexo_PRONIED.xlsx", firstrow(var) replace